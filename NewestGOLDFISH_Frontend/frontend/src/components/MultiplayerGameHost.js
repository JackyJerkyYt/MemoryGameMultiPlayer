...
function MultiplayerGameHost({ socket, friend, roomID }) {
  ...
  useEffect(() => {
    ...
    const handleOpponentFinished = (message) => {
      console.log(
        "Finisheddddddd",
        message,
        typeof message,
      ...
      if (message === "Both") {
        ...
        console.log("Now move to the next level");
      }
    ...
    socket.on("receive_finished", handleOpponentFinished);

    // Clean up
    return () => {
      socket.off("receive_finished");
      socket.off("receive_tiles");
      socket.off("receive_totalScores");
  ...
  const generateAnswerGrids = () => {
    ...
    console.log("Emitting the answer grid");
    socket.emit(
      "host_answer_grid",
      roomID,
      shuffled.slice(0, numberOfCorrectTiles),
      level
    );
  };
  const cleanTiles = () => {
    ...
    <>
      {win === true ? (
        ...
        <div>
          ...
          {finishedLevel ? (
            <div className="container">
              <h1>You pass this level! Waiting for the guest to proceed</h1>
            </div>
          ) : (
            <></>
          )}
...
export default MultiplayerGameHost;
